...
class Radar:
    ...
    def __init__(
        ...
        self.transmission_power = (
            transmission_power  # Transmission power (in Watts [W])
        )
        self.antenna_gain = antenna_gain  # Antenna gain (no units)
        self.wavelength = wavelength  # Wavelength (in meters)
        self.cross_section = (
            cross_section  # Cross-section of the antenna (in squared meters)
        )
    ...
    def compute_max_range(self) -> np.float64:
    ...
    def compute_detection_level(
        self, latitude: np.float32, longitude: np.float32
    ) -> np.float32:
        ...
        distance = (
            np.sqrt(
                (latitude - self.location.latitude) ** 2
                + (longitude - self.location.longitude) ** 2
            )
            * 111000
        )
        ...
        if distance <= max_range:
            ...
            discrepancy = (
                np.array([latitude, longitude]) - self.location.to_numpy()
            )  # x - mu
